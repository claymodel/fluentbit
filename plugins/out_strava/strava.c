...
static int cb_strava_init(struct flb_output_instance *ins,
                          struct flb_config *config, void *data)
{
    struct flb_strava *ctx;

    ctx = flb_strava_conf_create(ins, config);
    if (!ctx)
    {
...
static int strava_format(const void *in_buf, size_t in_bytes,
                         ...
                         struct flb_strava *ctx)
{
    ...
    while (msgpack_unpack_next(&result, in_buf, in_bytes, &off) == MSGPACK_UNPACK_SUCCESS)
    {
        ...
        if (ctx->strava_send_raw == FLB_TRUE)
        {
        ...
        /* Append the time key */
        msgpack_pack_str(&mp_pck, sizeof(FLB_STRAVA_DEFAULT_TIME) - 1);
        msgpack_pack_str_body(&mp_pck,
                              FLB_STRAVA_DEFAULT_TIME,
                              sizeof(FLB_STRAVA_DEFAULT_TIME) - 1);
        msgpack_pack_double(&mp_pck, t);

        if (ctx->strava_send_raw == FLB_FALSE)
        {
            /* Add k/v pairs under the key 'event' instead of to the top level object */
            msgpack_pack_str(&mp_pck, sizeof(FLB_STRAVA_DEFAULT_EVENT) - 1);
            msgpack_pack_str_body(&mp_pck,
                                  FLB_STRAVA_DEFAULT_EVENT,
                                  sizeof(FLB_STRAVA_DEFAULT_EVENT) - 1);
...
static void cb_strava_flush(const void *data, size_t bytes,
                            ...
                            struct flb_config *config)
{
    ...
    char *buf_data;
    size_t buf_size;
    struct flb_strava *ctx = out_context;
    ...
    ret = strava_format(data, bytes, &buf_data, &buf_size, ctx);
    if (ret == -1)
    {
    ...
    c = flb_http_client(u_conn, FLB_HTTP_POST, FLB_STRAVA_DEFAULT_URI,
...
static int cb_strava_exit(void *data, struct flb_config *config)
{
    struct flb_strava *ctx = data;

    flb_strava_conf_destroy(ctx);
    return 0;
}

struct flb_output_plugin out_strava_plugin = {
    .name = "strava",
    .description = "Send events to Strava HTTP Event Collector",
    .cb_init = cb_strava_init,
    .cb_flush = cb_strava_flush,
    .cb_exit = cb_strava_exit,
